...
If the user specifies a different install path, use that instead of `~/tradingview-mpc-jackson`.
...
```bash
cp ~/tradingview-mpc-jackson/rules.example.json ~/tradingview-mpc-jackson/rules.json
```
